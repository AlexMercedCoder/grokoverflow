...
![The Iceberg write process from data file creation through the atomic catalog pointer swap](/images/apache-iceberg-masterclass-06-writing-to-iceberg-write-process-flow.png)
...
For each new data file, the engine creates a manifest entry containing the file path, file size, row count, partition values (computed using the table's [partition transforms](/posts/2026/apache-iceberg-masterclass-05-hidden-partitioning)), and column-level statistics (min, max, null count).
...
The engine asks the [catalog](/posts/2026/apache-iceberg-masterclass-07-lakehouse-catalogs) to update its pointer from the old `metadata.json` to the new one. This is a compare-and-swap operation: the catalog checks that the current pointer matches what the engine expects, and only then updates it.
...
![How ACID works through the atomic metadata pointer swap](/images/apache-iceberg-masterclass-06-writing-to-iceberg-atomic-commit-pointer-swap.png)

The pointer swap mechanism delivers all four ACID properties:

**Atomicity.** The entire write is visible or invisible. If the engine crashes after writing data files but before the pointer swap, the data files are orphans. They exist in storage but no metadata references them. Readers never see partial writes. A cleanup process (covered in [Part 10](/posts/2026/apache-iceberg-masterclass-10-maintaining-iceberg)) can remove these orphans later.
...
![How two concurrent writers are resolved through optimistic concurrency with retry on conflict](/images/apache-iceberg-masterclass-06-writing-to-iceberg-concurrent-write-conflict.png)
...
Eventually, old snapshots are expired (removed from the metadata) and their orphan files are cleaned up. This maintenance is covered in [Part 10](/posts/2026/apache-iceberg-masterclass-10-maintaining-iceberg).
...
[Part 7](/posts/2026/apache-iceberg-masterclass-07-lakehouse-catalogs) covers the catalog landscape in detail.
