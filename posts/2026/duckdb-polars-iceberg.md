...
The write constraint to be aware of: DuckDB implements updates and deletes using positional deletes rather than full row rewrites (copy-on-write). For tables receiving heavy mutation loads, this means delete files accumulate between compaction runs, which is the same issue described earlier for Iceberg V2 CDC pipelines. For append-heavy analytical tables where DuckDB's primary use case lies, this is a non-issue.
...
One of the more surprising directions in the DuckDB ecosystem is its WebAssembly (Wasm) build, which is a version of DuckDB that runs entirely in the browser without any server component.
...
**Team size creates contention.** DuckDB and Polars run in-process. When a team of 20 analysts all need to run queries simultaneously, a shared distributed warehouse (such as Redshift, Snowflake, or Dremio) provides resource isolation and fair scheduling that single-process tools can't.
