...
This automated schema migration pattern (detecting DDL changes from Debezium and applying them to the Iceberg schema via the PyIceberg API) allows the CDC pipeline to self-heal after schema changes without manual intervention.
...
The Iceberg Flink connector commits data files to the Iceberg catalog at checkpoint time. This means that exactly-once semantics in the pipeline correspond to checkpoint frequency; for example, a 60-second checkpoint interval means the pipeline can be at most 60 seconds behind the latest committed snapshot in Iceberg. This is typically acceptable for analytical workloads but may need tuning for near-real-time requirements.
