...
**DatasetEvent:** Records a change to a dataset outside the context of a specific job, such as a schema change applied via DDL.

**JobEvent:** Records changes to a job definition, such as a DAG being updated in Airflow.

Each event carries **facets**, which are structured metadata payloads that extend the base event with specific information. The `SchemaDatasetFacet` records column names and types for a dataset. The `DataQualityMetricsInputDatasetFacet` records row counts and null rates for input datasets. The `SqlJobFacet` records the SQL query associated with a job. Facets are extensible: you can define custom facets for organization-specific metadata without breaking the standard format.
...
The most immediate operational benefit of a live lineage graph is blast radius analysis, which involves understanding what breaks when something changes.
...
For organizations that need enterprise governance features alongside lineage (such as access control, data classification, and business glossary), DataHub or commercial platforms like Atlan or Alation that support OpenLineage ingestion are the better fit.

---

## Conclusion

OpenLineage addresses the observability gap that has made data platform incidents disproportionately expensive to diagnose. A data pipeline fails in ways that are not visible to the tools monitoring individual components; Airflow knows the task failed, but does not know what the downstream effects are. Spark knows the job completed, but does not know what business dashboard depends on the table it wrote.
...
This approach treats SLA monitoring as a metadata query problem rather than an infrastructure monitoring problem: you are checking the lineage graph for expected events rather than polling health endpoints.
